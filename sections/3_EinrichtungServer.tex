...
Für die produktiven Systeme werden Red Hat Server genutzt, 
daher wird für den zentralisierten Logging-Server ebenfalls ein Red Hat Server installiert.
Es ist ein Red Hat Server mit der Version 7.9. \\
...
\textit{Satellite} ist ein Infrastruktur-Management Tool von Red Hat. 
Es ermöglicht die Verwaltung der Red Hat Hosts und bietet die \textit{rpm Packages} zum herunterladen an. \cite{redhatRedHatSatellite} \\
...
Sollte die Open-Source-Version heruntergeladen und installiert worden sein, dann ist es nicht möglich, ein Upgrade auf die Basic-Version zu vollbringen. 
...
Daher müssen die Installationspakete manuell vom Entwickler-Rechner heruntergeladen und anschließend zum Server kopiert werden. 
Da es sich um einen Red Hat Server handelt, werden \textit{rpm-Pakete} heruntergeladen und installiert.
Der Befehl, der genutzt wurde, um das Package herunterzuladen, ist: 
\begin{lstlisting}[language=bash]
wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.10.0-x86_64.rpm
\end{lstlisting}
Nachdem der Download fertiggestellt wurde, musste das \textit{rpm} package auf den Red Hat Server kopiert werden:
\begin{lstlisting} [language=bash]
...
Das Eintragen der IP-Adressen muss in \textit{network.host} vorgenommen werden.
...
Daher werden die einzelnen Schritte nicht noch einmal aufgeführt. \\

\subsection{Index Pattern}
Kibana ist im Vergleich zu Elasticsearch eine Webanwendung, die mit einem Internet Browser erreicht werden kann und von Anwendern in Zukunft genutzt wird. 
...
In Kibana wird KQL dazu genutzt, um die vorhandenen Logs nach bestimmten Kriterien zu filtern und die gewünschten Daten zu erhalten. 
...
Beide Varianten der gleichen Abfrage werden hier gezeigt: \\
\begin{lstlisting}
...
Mit Logstash können Logs gesammelt, geparsed und transformiert werden. 
Nachdem Logstash erfolgreich installiert wurde, fiel bei der Konfiguration auf, dass der Nutzen von Logstash durch die neu geschaffenen Logging-Richtlinien nicht mehr vorhanden ist. 
Denn der Vorteil von Logstash gegenüber Filebeat ist die Möglichkeit der Transformation der Logs. 
Mit Logstash können die Logdateien, die unstrukturiert sind, anhand von Pipelines in eine strukturierte neue Form gebracht werden. 
So können unstrukturierten Daten in strukturierte Daten umgewandelt werden. 
Durch die Logging-Richtlinien, an die sich das CFT Portale halten muss, besteht kein Nutzen für das transformieren von Logdateien. 
Die Logdateien werden immer im strukturierten Format abgespeichert. 
Da die Formatierung der Logs angemessen ist, müssen die Daten nur an den Elasticsearch-Service gesendet werden.
Für diese Aufgabe wird der leichtgewichtige Log-Agent Filebeat genutzt. 
In dem Szenario der Bachelorarbeit ist es nicht sinnvoll, Logstash zu installieren und zu konfigurieren, daher wurde auf die Einrichtung von Logstash verzichtet. \cite{elasticLogstashSammelnParsen}
...
Zu Beginn muss ein Zip-File heruntergeladen werden, das die Basic-Version von Filebeat enthält. 
...
\end{lstlisting}
Sollte es mehrere Elasticsearch-Services geben, können diese in das Intervall der Hosts mit eingetragen werden. 
So kann bei hoher Last zu mehreren Services gesendet werden. \\
...
Zeile 5 und 6 sind wichtig, denn diese ermöglichen das Einlesen von strukturieren Log-Daten im JSON-Format. 
...
\end{figure}
\newpage
Damit die Suche in Kibana effektiv durchgeführt werden kann, werden drei Index Pattern erstellt, jeweils für eine Stage. 
...
Filebeat sendet dafür automatisch die Informationen über den Hostnamen mit. 
