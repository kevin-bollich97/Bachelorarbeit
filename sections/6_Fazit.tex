...
Das Einrichten des zentralisierten Logging-Servers war nötig, weil der Prozess der Fehleranalyse sehr aufwendig war. 
...
Beim Installieren der Komponenten des Elastic Stacks ist aufgefallen, dass Logstash in dem Szenario nicht benötigt wird. 
Das Senden der Logs an den Elasticsearch Service wird von Filebeat durchgeführt. 
Mit Abschluss der Installationen wurden die Logging-Richtlinien an einer Anwendung umgesetzt. 
Die Anwendung, an der die Richtlinien umgesetzt worden sind, ist  die Vierteljahreserklärung. 
Damit die Richtlinien umgesetzt werden konnten, musste die Architektur und die fachlichen Aspekte der Vierteljahreserklärung näher erläutert werden. 
So konnten anschließend die relevanten Abschnitte des Quellcodes identifiziert werden.
Bevor die Log-Nachrichten an den richtigen Stellen platziert wurde, musste das NLog Framework konfiguriert werden. 
...
Der verbesserte Fehleranalyseprozess liefert ein zufriedenstellendes Ergebnis.
Wie erwartet ist der Aufwand der Fehleranalyse gesunken.  \\

% Wichtige Ergebnisse 
Das Ergebnis der Bachelorarbeit ist somit die erfolgreiche Einrichtung eines zentralisierten Logging-Servers, der den Arbeitsprozess der Fehlersuche des CFT Portale vereinfacht. 
...
Die genauen Schritten wurden in dem dazugehörigen Kapitel \ref{kap:einrichtung}vollständig erklärt. 
Die letzte Forschungsfrage beschäftigte sich mit Logging-Richtlinien, die in dieser Arbeit praktisch umgesetzt werden sollten. 
Auch diese Forschungsfrage konnte positiv beantwortet werden. 
Eine vollständige Umsetzung der Logging-Richtlinien an einem Projekt ist möglich.  \\

% Ausblick auf weitere Forschung
Im Anschluss an diese Bachelorarbeit sollen die aufgestellten Logging-Richtlinien in allen Applikationen des CFT Portale umgesetzt werden. 
Der aktuelle Stand bezieht sich nur auf die Vierteljahreserklärung. 
Dadurch ist bei der Fehlersuche in der Vierteljahreserklärung ein optimaler Ablauf möglich. 
