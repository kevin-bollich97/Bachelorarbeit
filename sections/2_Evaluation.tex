...
Das Installieren weiterer Log-Agenten, die für das Senden der Logs zuständig sind, sollte vermieden werden, da sonst weiterer Wartungsaufwand entstehen würde. \\
...
Jedoch möchte das Team das Tool gerne auf einem Linux-Server installieren, da dort das Updaten von neuen Versionen problemlos funktioniert. \\
...
Die Anforderungen werden nachfolgend zusammenfassend aufgelistet:
...
% VORAUSSETZUNGEN
Die Voraussetzungen für die Nutzung des Elastic Stack sind niedrig. 
Denn die einzelnen Komponenten des Elastic Stacks können auf den aktuell vorhandenen Betriebssystemen installiert werden. 
...
Es ist möglich, den Quellcode des Elastic Stack in öffentlichen Repositories zu betrachten. 
...
Elastic Stack bietet zu der selbstverwalteten Lösung zusätzlich eine Cloud-Lösung, die ohne Installation genutzt werden kann. 
Der Vorteil liegt im Wartungsaufwand, denn bei der Cloud-Lösung ist kein Installieren und Upgraden notwendig. 
Da die Anforderungen des Teams eine Cloud-Lösung ausschließen, wird diese nicht weiter beachtet. \cite{elasticOffiziellePreisinformationenElasticsearch} 
...
Die Funktionalitäten des Tools beziehen sich auf das Sammeln, Verbessern, Speichern und die Analyse von Logs. 
...
Damit muss keine neue Syntax erlernt werden und das Tool kann von unausgebildetem Personal genutzt werden. \cite{graylogIndustryLeadingLog} \\
...
Das bedeutet, dass diese Produkte zusätzlich auf der eingesetzten Maschine installiert werden müssen.  \\
...
Für Datenmengen bis zu 5 GB/Tag ist es daher ratsam, auf die Enterprise Variante zu setzen. \cite{graylogGraylogOpenSource} \\
\begin{figure}[H]
    \center 
...
% VORRAUSSETZUNGEN
Graylog benötigt einige Systemvoraussetzungen, um installiert werden zu können. 
...
Die größten Vorteile von Fluentd sind die Menge an verfügbaren Plugins und die Performance. 
...
Für das Senden von Logdateien bietet Graylog keinen eigenen Log-Agenten an.
Graylog empfiehlt, den Log-Agenten des Elastic Stacks zu nutzen, nämlich Filebeat. \cite{graylogIngestFilesGraylog} \\
...
Im Gegensatz zum Elastic Stack ist Sematext nicht kostenlos nutzbar.
Damit Sematext auf der eigenen Infrastruktur betrieben werden kann, muss Sematext Enterprise erworben werden. 
Durch den Erwerb der Sematext Enterprise Version, erhält man eine Kopie des Sematext Cloud Services, der auf der eigenen Infrastruktur installiert werden kann. 
...
Da kein ausschlusskriterium für eines der Tools vorliegt und die Tools annähernd identische Funktionen anbieten, wird im Rahmen dieser Bachelorarbeit Logstash verwendet. 
Da Logstash Teil des bereits gewählten Elastic Stack ist und somit kein weiteres Tool installiert werden muss, wird Logstash verwendet. 
...
\input{tables/vergleichTools.tex}