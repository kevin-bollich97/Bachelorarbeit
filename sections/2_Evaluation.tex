...
Das bedeutet, dass das Tool eine Lösung bieten muss, die auf den Servern der KV laufen sollen. \\
...
Da das CFT Portale keine Datenbanken betreuen muss, ist der aktuelle Wissensstand des Teams daher eher allgemein vorhanden. 
Denn das Team übernimmt in der Regel die Entwicklung von Provider-hosted Apps im SharePoint Umfeld. 
Damit das Team also erfolgreich mit den Datenbanken arbeiten kann, müssen Schulungen absolviert werden. 
Daher ist es wichtig, dass das ausgewählte Tool eine Möglichkeit bietet, die Logs zu speichern. \\
...
Jedoch möchte das Team das Tool gerne auf einem Linux-Server installieren, da dort das Updaten von neuen Versionen einfacher funktioniert und der Wissensstand des Teams da komplett gegeben ist. \\
...
Zur veranschaulichung der Vorgaben folgt eine Aufzählung:

\begin{itemize}
    \item Selbstorganisierte Lösung
...
Jedoch wird in diesem Kapitel auf die wichtigen Inhalte des Elastic Stack eingegangen, um eine Bewertung der Tools zu ermöglichen.
...
Die KernprodukteTools sind Elasticsearch, Kibana, Logstash und Beat. 
...
In einem Ranking von Searchengines wurde Elasticsearch als erster Platz ausgezeichnet und beweist damit, dass Elasticsearch schnell und Effizient mit großen Datenmengen umgehen kann. \cite{db-enginesDBEnginesRanking} \\
\newpage
...
Denn die einzelnen Komponenten des Elastic Stack können auf den aktuell vorhanden Betriebssystemen installiert werden. 
Zusätzlich zu den Komponenten des Elastic Stack wird nur noch eine Installation von Java benötigt.
...
% KOSTEN
Die Nutzung des Elastic Stack ist grundsätzlich Kostenlos und ist zu einem großen Teil Open Source.
Es ist möglich den Quellcode des Elastic Stack in öffentlichen Repositories betrachtet. 
Der Elastic Stack kann ebenfalls als Open Source Variante installiert werden, jedoch ist die Nutzung der Basic Version nicht möglich. 
Wichtige Features, wie Sicherheitsmechanismen sind nicht Nutzbar. 
Daher ist es ratsam die kostenlose Basic Version mit mehr Features zu nutzen. 
...
Der Vorteil der Cloud lösung liegt am Wartungsaufwand, denn bei der Cloud Lösung ist kein Installieren und upgraden notwendig. 
Da die Anforderungen des Teams eine Cloud Lösung ausschließt, wird die Cloud Lösung nicht weiter beachtet. \cite{elasticOffiziellePreisinformationenElasticsearch} 
...
Damit muss keine neue Suchsprache erlernt werden und kann von unausgebildetem Personal genutzt werden. \cite{graylogIndustryLeadingLog} \\
...
Der genaue Vergleich der beiden Varianten kann in Abbildung \ref{fig:vergleichGraylogOSvsE} gesehen werden.
...
Bei den 5 GB handelt es sich um Logs die gesendet werden. 
...
% VORRAUSSETZUNGEN
Graylog benötigt ein paar System Vorraussetzungen um installiert werden zu können. 
...
Die zu installierende Software ist: // ZITIEREN -> https://docs.graylog.org/en/4.0/pages/installation.html
\begin{itemize}
...
An der Auflistung sieht man, dass spezielle Versionen der Tools installiert sein müssen, damit Graylog funktionieren kann. 
Dies kann bei der Wartung zu Problemen führen. 
Denn es muss beim Updaten auf neuere Versionen darauf geachtet werden, dass alle Versionen miteinander kompatibel sind.
Daher ist das Updaten von Graylog nicht so einfach möglich.
Ein Problem ist auch die veraltete Java Version, die von Graylog genutzt wird. 
Aktuell ist es schon möglich Graylog mit Java 11 zum laufen zu bringen, jedoch ist es noch in der Testphase.  
Das Updaten wird dadurch nicht vereinfacht und erfordert zusätzliche Arbeit.
...
Deswegen benötigt es nur wenig System Ressourcen.
...
Die größten Vorteile von Fluentd sind die Menge an Plugins die verfügbar und die Performance. 
Außerdem verbraucht Fluentd sehr wenig Speicher. 
Der größte Nachteil ist der Zwang von strukturierten Daten. 
...
Der Elastic Stack benötigt zum ausführen nur die Tools vom Elastic Stack. \\
Beim Vergleich von Graylog und dem Elastic Stack fällt auf, dass der Elastic Stack insgesamt die bessere Entscheidung darstellt, weil keine weitere Software installiert werden muss. 
Dadurch ist der Wartungsaufwand geringer. 

\subsection{Sematext vs. Elastic Stack}

Sematext bietet eine vollständig installierte Version vom Elastic Stack an. 
Das bedeutet, die Vor- und Nachteile sind identisch, da sie an Funktionalitäten das selbe anbieten. 
Jedoch ist der Vorteil von Sematext, dass der Support mit angeboten wird und das Konfigurieren und Einrichten nicht so komplex ausfällt wie beim Elastic Stack selber. 
Ein großes Problem von Sematext stellt die Installation der Enterprise Variante dar. 
Denn die Installation erfolgt über Kubernetes und Docker Container. 
Im CFT Portale besitzt kein Teammitglied das nötige Wissen, um die Installation und Wartung zu übernehmen. 
...
Im Vergleich zu Logstash ist Fluentd sehr viel performanter und verbraucht weniger Speicherplatz. 
Jedoch sind die zwei Vorteile nicht relevant für den Einsatz im CFT Portale. 
Ressourcen sind genügend vorhanden. \\
...
Bei der Evaluation ist klar geworden, dass der Elastic Stack die beste Wahl für einen zentralisierten Logging Server für das CFT Portale ist. 
