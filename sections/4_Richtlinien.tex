...
§7 Absatz 1 der Abrechnungsrichtlinien beschreibt die Pflichten des Vertragsarztes bezüglich der \textit{Erklärung der Vierteljahresabrechnung} so:  
...
Mithilfe der Anwendung \textit{Vierteljahreserklärung} können die Angaben, die für die Erklärung zur Vierteljahresabrechnung notwendig sind, eingetragen werden. 
...
Nachdem ein Quartal ausgewählt wurde, werden die gültigen Betriebsstättennummern des Vertragsarztes, die für den Zeitraum gültig sind, in die Anwendung geladen.
Dabei wird geprüft, an welchen Betriebsstätten der Vertragsarzt in dem Quartal tätig war. 
Anschließend kann eine BSNR gewählt werden, für die eine Erklärung erstellt werden soll. 
...
\begin{figure}[H]
    ...
    \label{fig:vje}
\end{figure}
\noindent Die App \textit{Vierteljahreserklärung} besteht aus einer C\#-Anwendung und einer Angular-Anwendung. 
...
Damit beide Anwendungen miteinander kommunizieren können, wird in der C\#-Anwendung eine Schnittstelle zur Verfügung gestellt.
...
Die Parameter sind das Quartal und das zugehörige Jahr, für die die passenden Betriebsstättennummern geladen werden sollen. \\
Der Aufruf der GET-Anfrage:\textit{\glqq /api/bsnrs\grqq{}} ruft die C\#-Methode \textit{GetBSNRsAsync} auf.
\textit{GetBSNRsAsync} liefert die gültigen Betriebsstättennummern für ein bestimmtes Quartal.  
...
Die aus der Arztregister-Schnittstelle erhaltenen Daten sind stark verschachtelt und enthalten viele Informationen, die für die Vierteljahreserklärung irrelevant sind. 
Daher werden die Daten in DTOs (Data Transfer Objects) geladen, um die Menge an HTTP-Anfragen zu reduzieren. 
Mit den DTOs können dann mehr Daten über einen Aufruf gesendet werden. 
Die über das Netzwerk gesendeten Daten werden so gering wie möglich gehalten, um den Mitgliedern mit einer schwachen Netzanbindung, die Nutzung der Anwendung zu ermöglichen. 
...
Die vorgestellten Log-Aufrufe wurden alle in der Vierteljahreserklärung umgesetzt. 
Die Logs werden zusätzlich in die Kategorien eingeordnet, die in der Projektarbeit vorgestellt wurden. 
Die in der Projektarbeit erarbeiteten Kategorien sind: \cite{bollichErarbeitungLoggingRichtlinienFuer2020}

\begin{itemize}
  \item Program operation
    \begin{itemize}
      \item completed operation
      \item current operation
      \item next operation
    \end{itemize}
  \item Error message
    \begin{itemize}
...
Die Arztregister-Schnittstelle wird vom CFT Sicherstellung verwaltet. 
...
Dieser Log-Aufruf kann zu der Kategorie \textit{Semantic Description} zugeordnet werden. 
Wie in der Auflistung zu Beginn des Kapitels zu sehen ist, wird die Kategorie \textit{semantic description} in drei weitere Unterkategorien eingeteilt. 
Der in Quellcode \eqref{code:aufrufArztregister} gezeigte Log-Aufruf kann der Unterkategorie \textit{variable description} zugeordnet werden. 
Das Log, das mit diesem Aufruf geschrieben wird, enthält der Inhalt einer Variable. 
Mit diesem Log wird geprüft, ob die Variable den gewünschten Wert beinhaltet.\cite{bollichErarbeitungLoggingRichtlinienFuer2020}
...
Daher werden in der Vierteljahreserklärung Error-Messages geloggt. 
Dafür wird das Log-Level \textit{Error} genutzt. 
...
Da in dem Quellcode \ref{code:auftretenFehler} eine Variable geprüft wird und anschließend eine Fehlermeldung geworfen wird, gehört der dabei entstehende Log-Aufruf zu der Kategorie \textit{Error Message}.
Wegen dem Prüfen einer Variable wird dieser Log-Aufruf der Unterkategorie \textit{value-check} zugeordnet.
...
Quellcode \ref{code:aufrufREST} zeigt die Methode \mbox{\textit{GetBSNRsAsync}}. 
Diese Methode wird aufgerufen, wenn ein Quartal in der Vierteljahreserklärung ausgewählt wurde. 
Der Client sendet die Anfrage, um die gültigen Betriebsstättennummern des Mitglieds für den ausgewählten Zeitraum zu erhalten.
Das dazu geschriebene Log befindet sich zu Beginn dieser Operation, um zu definieren, dass der Aufruf der Methode funktioniert hat. \\

Da das Log zu Beginn der Operation platziert wurde, gehört dieses Log zu der Kategorie \textit{program operation}.
Die Operation die dieser Log-Aufruf beschreibt, wird nach dem Log-Aufruf ausgeführt, somit beschreit der Log-Aufruf eine Funktion die anschließend durchgeführt wird.
Daher kann dieser Log-Aufruf der Unterkategorie \textit{next operation} zugeteilt werden. 
